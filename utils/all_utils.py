...
def prepare_data(df):
    ...
    Returns:
        tuple: it returns the tuples of dependent vatiables and independent variables
    """
    logging.info("Praparing the data for model training.")
    X = df.drop("y", axis=1)
    y = df["y"]
    return X, y
...
def save_plot(df, file_name, model):
    def _create_base_plot(df):
        logging.info("saving the basic plot")
